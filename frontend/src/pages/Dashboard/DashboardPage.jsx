import { useState, useMemo } from 'react';import { useAuth } from '../../features/auth/hooks/useAuth';
...
import { useLeetcodeDashboard, useCodeforcesDashboard } from '../../features/dashboard/hooks/usePlatformDashboard';
...
export default function DashboardPage() {
  ...
  const handleSync = async () => {
    if (platform === 'leetcode') {
      ...
      try { await syncLeetCode.mutateAsync(); } catch { /* toast handled in hook */ }
    } else {
      if (!profile?.codeforcesUsername) {
        toast.error('Set your Codeforces handle in your profile first');
        return;
      }
      try { await syncCodeforces.mutateAsync(); } catch { /* toast handled in hook */ }
    }
  };

  const syncing = platform === 'leetcode' ? syncLeetCode.isPending : syncCodeforces.isPending;
            ...
            <div className="flex items-center gap-2 h-9">
              ...
              >
                {copied
                  ...
                  : <><Copy size={14} /> Share /u/{profile?.lucyUsername || profile?.leetcodeUsername || 'user'}</>
                }
              </Button>
              <button
                type="button"
              ...
              >
                ...
                {syncing
                  ? 'Syncing…'
                  : platform === 'leetcode' ? 'Sync LeetCode' : 'Sync Codeforces'}
              </Button>
            </div>
          </div>
...
function LeetCodeView({ data, profile }) {
  ...
  const heatmapData = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const k = `${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}-${String(d.getUTCDate()).padStart(2,'0')}`;
      cells.push({ date: d.toLocaleDateString(), count: dayMap[k] || 0 });
    }
    return cells;
  ...
  const badges = computeLeetcodeBadges(displayStats);

  return (
    <>
      {/* Metrics row */}
      <section className="grid grid-cols-2 md:grid-cols-4 gap-5 mt-10" data-testid="lc-metrics">
        <Metric label="Total Solved"    value={displayStats.total_solved}              sub={`#${displayStats.global_ranking?.toLocaleString() || '-'} global`} />
        <Metric label="Contest Rating"  value={displayStats.contest_rating?.toFixed(0) || 0} sub={`${displayStats.contests_attended} contests`} mono />
        <Metric label="Current Streak"  value={`${displayStats.streak}d`}             sub={`${displayStats.active_days} active days in 30d`} />
        <Metric label="LeetCode Score"  value={Math.round(displayStats.leetcode_score || displayStats.universal_score)} sub="lucy V1 score" />
      </section>

      {/* Charts row */}
      <section className="grid lg:grid-cols-5 gap-5 mt-6">
        <Card title="Difficulty Breakdown" subtitle="Easy / Medium / Hard" testid="lc-card-difficulty" className="lg:col-span-2">
            ...
            <div className="col-span-3 h-56">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  >
                    {['#10B981','#F59E0B','#F43F5E'].map((c,i) => <Cell key={i} fill={c} />)}
                  </Pie>
        ...
        <Card title="Rating Trend" subtitle={`${displayStats.contests_attended} contests attended`} testid="lc-card-rating" className="lg:col-span-3">
          {displayStats.rating_history.length > 0 ? (
            <RatingChart history={displayStats.rating_history} gradientId="lcRatingGrad" />
          ) : (
      ...
      {/* Heatmap + Badges */}
      <section className="grid lg:grid-cols-5 gap-5 mt-6">
        <Card title="30-Day Activity" subtitle="Submissions per day" testid="lc-card-heatmap" className="lg:col-span-3">
          <Heatmap cells={heatmapData} />
        </Card>
        <Card title="Badges" subtitle={`${badges.length} earned`} testid="lc-card-badges" className="lg:col-span-2">
...
function CodeforcesView({ data, profile }) {
  ...
  return (
    <>
      {/* Metrics row */}
      <section className="grid grid-cols-2 md:grid-cols-4 gap-5 mt-10" data-testid="cf-metrics">
        <Metric label="CF Rating"         value={stats.rating       || 0}  sub={<span className={`font-medium ${cfRankColor(stats.rank)}`}>{stats.rank || 'unrated'}</span>} />
        <Metric label="Max Rating"        value={stats.maxRating    || 0}  sub={stats.maxRank || '–'} mono />
        <Metric label="Problems Solved"   value={stats.totalSolved  || 0}  sub={`${stats.totalSubmissions || 0} submissions`} />
        <Metric label="Codeforces Score"  value={Math.round(stats.codeforcesScore || 0)} sub="lucy V1 score" />
      </section>

      {/* Stats row */}
      <section className="grid grid-cols-2 md:grid-cols-3 gap-5 mt-6">
        <Metric label="Contests Attended" value={stats.contestsAttended || 0} sub="rated contests" />
        <Metric label="Contribution"      value={stats.contribution    || 0} sub="CF contribution" mono />
        <Metric label="Friend Of"         value={stats.friendOfCount   || 0} sub="users" />
      </section>

      {/* Rating trend */}
      <section className="grid lg:grid-cols-5 gap-5 mt-6">
        <Card title="Rating History" subtitle={`${stats.contestsAttended || 0} contests`} testid="cf-card-rating" className="lg:col-span-3">
        ...
        <Card title="Top Tags" subtitle="by problems solved" testid="cf-card-tags" className="lg:col-span-2">
      ...
      <section className="grid lg:grid-cols-2 gap-5 mt-6">
        <Card title="Rating-wise Solved" subtitle="unique problems by difficulty" testid="cf-card-rating-wise">
          <RatingWiseList buckets={stats.ratingWiseSolved || []} />
        </Card>
        <Card title="Language Usage" subtitle="by submission count" testid="cf-card-languages">
...
function ConnectPrompt({ platform, message, testid }) {
  return (
    <div className="mt-14 flex flex-col items-center text-center gap-4" data-testid={testid}>
      <div className="h-16 w-16 rounded-full bg-muted border border-border flex items-center justify-center">
        {platform === 'LeetCode' ? <Code2 size={28} className="text-muted-foreground" /> : <Cpu size={28} className="text-muted-foreground" />}
      </div>
      <div>
        <div className="font-heading text-xl text-foreground">{platform} not connected</div>
        <div className="text-sm text-muted-foreground mt-1 max-w-xs">{message}</div>
      </div>
...
function SyncPrompt({ platform, message, testid }) {
  return (
    <div className="mt-14 flex flex-col items-center text-center gap-4" data-testid={testid}>
      ...
      <div>
        <div className="font-heading text-xl text-foreground">{platform} not synced yet</div>
        <div className="text-sm text-muted-foreground mt-1 max-w-xs">{message}</div>
      </div>
    </div>
  );
...
function RatingChart({ history, gradientId = 'ratingGrad' }) {
  const data = history.map((e) => ({
    ...e,
    date: new Date((e.timestamp || e.ratingUpdateTimeSeconds || 0) * 1000)
    ...
    <div className="h-56">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top:5, right:10, left:-10, bottom:0 }}>
          ...
          <XAxis dataKey="date" tick={{ fontSize:11, fill:'hsl(var(--muted-foreground))' }} axisLine={{ stroke:'hsl(var(--border))' }} tickLine={false} interval="preserveStartEnd" />
          <YAxis tick={{ fontSize:11, fill:'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} domain={['dataMin - 50','dataMax + 50']} />
          <Tooltip
            ...
            labelFormatter={(l, p) => p?.[0]?.payload?.contest || p?.[0]?.payload?.contestName || l}
          />
          <Area type="monotone" dataKey="rating" stroke="hsl(var(--primary))" strokeWidth={2}
            fill={`url(#${gradientId})`}
    ...
    <>
      ...
      <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <div className="flex gap-1">
          {[0,1,2,3,4].map((l) => <div key={l} className={`heat-cell ${l ? 'heat-'+l : ''}`} />)}
        </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {badges.map((b) => {
        ...
        return (
          <div key={b.name} className={`border rounded-md px-3 py-2.5 flex items-center gap-2.5 ${TIER_STYLES[b.tier] || TIER_STYLES.silver}`}>
...
function TagList({ tags }) {
  ...
  if (!top.length) return <div className="text-sm text-muted-foreground">No tag data yet.</div>;
  const max = top[0]?.solved || 1;
  return (
    <div className="space-y-2">
      {top.map((t) => (
        <div key={t.tag} className="flex items-center gap-3">
          <div className="text-xs text-muted-foreground w-28 truncate">{t.tag}</div>
          <div className="flex-1 bg-muted rounded-full h-1.5 overflow-hidden">
            <div className="bg-primary h-full rounded-full" style={{ width: `${(t.solved / max) * 100}%` }} />
          </div>
          <div className="text-xs font-mono-display text-muted-foreground w-6 text-right">{t.solved}</div>
        </div>
      ))}
    </div>
  );
...
function RatingWiseList({ buckets }) {
  if (!buckets.length) return <div className="text-sm text-muted-foreground">No rating data yet.</div>;
  ...
  const bucketColor = (rating) => {
    ...
    if (rating >= 2100) return { bar: 'bg-orange-500', label: 'text-orange-400' };
    if (rating >= 1900) return { bar: 'bg-violet-500', label: 'text-violet-400' };
      ...
      {buckets.map((b) => {
        ...
        return (
          <div key={b.rating} className="flex items-center gap-3">
            <div className={`text-xs font-mono-display w-12 text-right shrink-0 ${label}`}>
              {b.rating}
            </div>
            <div className="flex-1 bg-muted rounded-full h-1.5 overflow-hidden">
              <div
...
function LanguageList({ languages }) {
  ...
  if (!top.length) return <div className="text-sm text-muted-foreground">No language data yet.</div>;
  const max = top[0]?.count || 1;
  return (
    <div className="space-y-2">
      {top.map((l) => (
        <div key={l.language} className="flex items-center gap-3">
          <div className="text-xs text-muted-foreground w-28 truncate">{l.language}</div>
          <div className="flex-1 bg-muted rounded-full h-1.5 overflow-hidden">
            <div className="bg-blue-500 h-full rounded-full" style={{ width: `${(l.count / max) * 100}%` }} />
          </div>
          <div className="text-xs font-mono-display text-muted-foreground w-8 text-right">{l.count}</div>
        </div>
      ))}
    </div>
  );
...
function ProfileFooter({ profile, stats, platform }) {
  const handle = platform === 'leetcode' ? profile?.leetcodeUsername : (stats?.handle || profile?.codeforcesUsername);
  const url    = platform === 'leetcode'
    ? `https://leetcode.com/${handle}/`
    : `https://codeforces.com/profile/${handle}`;
  const label  = platform === 'leetcode' ? 'View on LeetCode' : 'View on Codeforces';
  const avatar = stats?.avatar || profile?.avatar || `https://api.dicebear.com/7.x/initials/svg?seed=${profile?.name || 'U'}`;

  return (
    <section className="mt-6 bg-card border border-border rounded-md p-6 flex flex-col md:flex-row md:items-center gap-5">
      <img src={avatar} alt="" className="h-14 w-14 rounded-full border border-border" />
