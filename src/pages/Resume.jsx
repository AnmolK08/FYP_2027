...
export default function Resume() {
  ...
  useEffect(() => { api.getResumeRoles().then(r => setRoles(r.roles)).catch(()=>{}); }, []);
  const score = async () => {
    setBusy(true);
    try { const result = await api.scoreResume(text, role); setResult(result); }
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 py-10">
        ...
        <div className="mt-8 grid lg:grid-cols-2 gap-6">
          <div className="ps-card">
            ...
            <button onClick={score} disabled={!text.trim()||busy} data-testid="resume-score" className="btn-primary mt-3"><FileText size={14}/>{busy?'Scoring…':'Score my resume'}</button>
          </div>
